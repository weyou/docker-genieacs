...
if [ ! -z "$FS_IP" ]; then
    if [ "$FS_IP" = "<dynamic>" ]; then
        # get host ip from host.docker.internal
        while true; do
            FS_IP=$(ping -nc 1 host.docker.internal | sed -n "s/.* host.docker.internal (\([^)]\+\)).*/\1/p")
            if [ ! -z "$FS_IP" ]; then
                break
            fi
            echo "Wait 5 minutes to get address of host.docker.internal"
            sleep 5
        done
    fi
